...
Though we're merely wandering at the edge of the work being done, we'll see how to use category theory to build the specifications for, and simulate the behaviors of, dynamical systems built from the composition of smaller, simpler systems.
We'll do this by modeling a gene transcription network as the composition of individual differential systems.
...
\[\mathcal{C} := \begin{tikzcd}
    ...
    \end{tikzcd}\]
\\\\It's dual category, $\mathcal{C}^{op}$, is the following.
...
We'll now look at the kinds of structures that we can find within a category.
...
Contrast this to $30$ which also has $3$ and $5$ as divisors but also $2$: this extra divisor is superfluous and makes $30$ a worse candidate for $3*5$ than $15$.
...
For our purposes we'll be sticking to one such cartesian category: the category of lenses over the category of Euclidean spaces and smooth functions, $\textbf{Lens}_\textbf{Euc}$.
