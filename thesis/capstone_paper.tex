...
\begin{definition}[Functor]
    ...
    \begin{enumerate}
        ...
        \item For all objects $a\in\mathcal{C}$ and $Fa\in\mathcal{D}$, $Fid_a=id_{Fa}$.
...
\begin{itemize}
    ...
    \item $N_a:Fa\rightarrow Ga$; $N_a = id_x$
    \item $N_b:Fb\rightarrow Gb$; $N_b = k$
    \item $N_c:Fc\rightarrow Gc$; $N_c = id_y$
\end{itemize}
...
We'll start with three types of morphisms that tell us something about the objects the map between.


\subsection*{Kinds of Morphisms}
The first and most important kind of morphism is an isomorphism.
In the category of sets and functions these morphisms are exactly the bijective functions.
More generally, the existence of an isomorphism between objects in a category tells us that it's possible to move from one object to the other and then back again without losing any information in the process.
We say these two objects are the same up to isomorphism, or isomorphic.
\color{black}
\begin{definition}[Isomorphism]
    Two objects in a category are isomorphic if there exists a morphism $f:a\rightarrow b$ for which there is another morphism $f^{-1}:b\rightarrow a$ such that $g\circ f=id_b$ and $f\circ g=id_a$.
...
\begin{definition}[Monad]
    ...
    \begin{enumerate}
        \item $\eta : id_{\mathcal{C}} \Rightarrow M$ called the unit.
    ...
    \[\begin{tikzcd}
            ...
            \arrow["\mu", from=1-3, to=3-3]
            \arrow["{id_M}", from=1-5, to=3-3]
            \arrow["{id_M}"', from=1-1, to=3-3]
        \end{tikzcd}\]
...
\begin{definition}[Monoidal Categories]
    ...
    \[\begin{tikzcd}
            ...
            \\
            && {x\otimes y}
            \arrow["{{\rho_x\otimes id_y}}"', from=1-1, to=3-3]
            \arrow["{{id_x\otimes\lambda_y}}", from=1-3, to=3-3]
    ...
    \[\begin{tikzcd}
            ...
            \arrow["{{\alpha_{w,x,y}\otimes id_z}}"', from=1-1, to=3-1]
            \arrow["{{\alpha_{w,(x\otimes y),z}}}"', from=3-1, to=5-1]
            \arrow["{{id_w\otimes \alpha_{x,y,z}}}"', from=5-1, to=5-3]
        \end{tikzcd}\]
\end{definition}
\begin{definition}[Symmetric Monoidal Category]
    A monoidal category equipped with a natural isomorphism $\beta:a\otimes b\Rightarrow b\otimes a$, called its braiding, is symmetric if $\beta_{y,x}\circ\beta_{x,y}=id_{x\otimes y}$ for all $x,y\in\mathcal{C}$
    ...
    \[\begin{tikzcd}
            ...
            \arrow["{\beta_{x,y}\otimes id_z}"', from=1-1, to=4-1]
            \arrow["{\alpha_{y,x,z}}"', from=4-1, to=4-3]
            \arrow["{id_y \otimes \beta_{x,y}}"', from=4-3, to=4-5]
        \end{tikzcd}\]
\end{definition}
