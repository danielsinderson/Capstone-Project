...
Taking one more step into the rarefied air, it's also possible to create a category whose objects are functors and whose structure-preserving maps are structures called natural transformations.
...
\begin{definition}[Adjunctions]
    ...
    Formally, that means that the following holds:
    \begin{enumerate}
        \item $\mathcal{C}(c,Gd)\cong\mathcal{D}(Fc,d)$
        \item ...
    \end{enumerate}
\end{definition}
